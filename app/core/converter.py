"""
Конвертер файлов в изображения для PaddleOCR-VL.
PDF → pdf2image; DOCX → LibreOffice → PDF → PNG; фото → предобработка при необходимости.
"""
import os
import subprocess
import tempfile
...
def _preprocess_image(img: Image.Image) -> Image.Image:
    """Предобработка для OCR: черно-белое (опционально бинаризация), апскейл мелких, резкость."""
    # Черно-белое: в оттенки серого, затем обратно в RGB (модель ожидает 3 канала)
    if PREPROCESS_GRAYSCALE or PREPROCESS_BINARIZE:
        if img.mode != "L":
            img = img.convert("L")
        if PREPROCESS_BINARIZE:
            # Чисто ч/б по порогу 127
    ...
    w, h = img.size
    min_side = min(w, h)
    if min_side > 0 and min_side < IMAGE_MIN_SIDE:
    ...
    if PREPROCESS_SHARPNESS != 1.0:
        ...
        img = enhancer.enhance(PREPROCESS_SHARPNESS)
