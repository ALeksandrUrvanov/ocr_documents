...
Асинхронная обработка документов:
    POST /api/documents/upload → task_id
    GET  /api/documents/{id}/status → прогресс обработки
    GET  /api/documents/{id}/result → JSON результат (удаляется из памяти после выдачи)
"""
import logging
import uuid
import shutil
...
async def _save_and_queue(file: UploadFile) -> tuple[str, str, int]:
    """Сохранение файла и добавление в очередь обработки."""
    ...
    task_queue.add_task(task_id, safe_filename)
...
async def upload_document(file: UploadFile = File(...)):
    ...
    Поддерживаемые форматы: PDF, DOCX, PNG, JPG, JPEG, TIFF, BMP.
    Максимальный размер: 200 MB.
    """
    task_id, filename, size = await _save_and_queue(file)
...
async def get_status(task_id: str):
    ...
    Возвращает текущий статус и прогресс обработки (страницы).
    """
...
async def get_result(task_id: str):
    ...
    ВАЖНО: После выдачи результат удаляется из памяти.
    Повторный запрос вернёт 404.
    """
    result = task_queue.get_result(task_id)
    if result is None:
    ...
    # Успешное завершение - удаляем результат и возвращаем
